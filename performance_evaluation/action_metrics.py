...
def plot_action_AP(resultFile):
    ...
    # ======================= metrics ============================
    precision, recall, threshold = metrics.precision_recall_curve(labels, pred_class)

    fpr, tpr, thresholds = metrics.roc_curve(labels, pred_class)

    # ROC = metrics.roc_auc_score(labels,pred_possible)  # 梯形块分割，建议使用
    AP = metrics.average_precision_score(labels, pred_class)  # 小矩形块分割
    ROC = metrics.roc_auc_score(labels, pred_class)
...
def create_action_result():
    # 加载测试数据
    test_dataset = PoseDataSet(r'/home/kls/data/human_pose', False)
    test_dataLoader = DataLoader(test_dataset, batch_size=1, shuffle=True)

    net = NetV2()
    # 加载已训练的数据
    net.load_state_dict(torch.load("../action_detect/checkpoint/action.pt"))
    net.to(DEVICE)  # 使用GPU进行训练

    # labels = []
    # pred = []

    with open('../performance_evaluation/action_result.txt', 'a') as f:
...
if __name__ == '__main__':
    ...
    plot_action_AP('../performance_evaluation/action_result.txt')
